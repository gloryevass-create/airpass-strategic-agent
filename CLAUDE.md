...
- 메일 본문 HTML은 사용자가 Claude Design으로 만든 템플릿을 그대로 이식했다(`lib/materialEmailTemplate.ts::buildMaterialEmailHtml`,
  ...
  - **맺음말·맺음 인사**: 본문 끝의 "검토 중 궁금하신 사항이…" 문단과 "감사합니다. /
    주식회사 에어패스" 인사도 같은 이유로 폼 입력란으로 뺐다(2026-09-14). 인사는
    여러 줄로 쓸 수 있고 **마지막 줄만 굵게** 나온다(기존 모양 유지 — 폼 라벨에도
    그렇게 적어뒀다). 둘 다 비우면 해당 문단이 통째로 빠진다.
